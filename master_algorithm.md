...
## 类推学派    
- 类比学习法    
1. 最近邻算法，每个数据点就是自己的微型分类器，K最近邻算法    
2. 支持向量机，相似性是机器学习的核心思想之一    
3. 成熟的类比推理法    
也许在未来10点，机器学习会被深度类比统治，在某种算法中，结合了最近邻法的高效、支持向量机的数学精密性、类比推理的力量和灵活性    

- 懒惰学习算法会胜出的原因在于，和构建全局模型（例如决策树）相比，只要每次弄明白指定的点在哪里会较为简单    
构建局部模型而非全局模型的想法可以应用于分类之外的用途    

- 最近邻算法的维度灾难    
受到相关属性的迷惑    
最近邻算法的基础是找到相似物体，而在高维度情况下，相似性的概念就会无效，超空间就像过渡区域    
“缩减”不那么相关的属性，首先会舍弃那些信息增益低于阈值的属性，然后只在简化的空间中测量相似性    

- 支持向量机    
只记住那些用于确定边界的关键例子    
看起来很像加权k最近邻算法，正类与负类之间的边界由一组例子，其权值加上相似测度来确定    
相似性度量，在支持向量机领域被称为“核心程序”    

认知科学见证了符号学派（指向他们能够模仿但类推学派无法模仿的东西）与类推学派很长一段时间的争论    



## 进化学派  
- [罗纳德费雪](https://baike.baidu.com/item/%E7%BD%97%E7%BA%B3%E5%BE%B7%C2%B7%E8%B4%B9%E9%9B%AA/10178543?fr=aladdin)《自然选择的遗传理论》    
关于进化的第一套数学理论    
但孤立的看待每个基因    

- 约翰霍兰德    
有机体的适应度是它所有函数的复值函数    
遗传算法（有点像选择育种），关键输入就是一个适应度函数（给定一个待定程序和某个设定的目标，适应度函数会给程序打分，反应它与目标的契合度）    
为每一代中两个适应力最强的个体进行配对，通过随机交叉父母位串中的一点，让每队父母生出两个后代，评分，后代比前代适应度更高，直到达到理想的适应度或时间用尽
“作弊”的方法－适应力很强的可以与“儿子”、“孙子”等竞争，“不朽性”    

- 类似于进化的“间断平衡”（／“跳跃式进化”），遗传算法表现类似，原因如下：    
一旦算法达到适应度的局部最大值，算法会在这一点停很长时间，直到某次幸运的变异或者交叉，让处于坡上的个体登到更高的顶峰，在这一点上该个体会进行大量繁殖，按后和过往的每一代来爬上这个坡。    

- 遗传算法vs多层感知器    
1. 遗传算法    
每一步都考虑整体群体的假设，且可传到下一代    
对即将学习的结构不会进行预先假设    
陷入局部最优的可能性小，原则上更能找到新东西（真正的），分析起来困难得多    
2. 多层感知器    
给定时间内坚持单一假设，该假设渐渐改变，直到某个局部最优值    
预先设定网络结构，调整权值    

- 遗传编程    
对程序树，而不是字符串进行交叉，第一次成功是在1995年    

- 演化新论者（交叉／性>突变）vs联结学派    
共同点：受自然启发而设计了学习算法    
区别：前者关注学习架构，后者更喜欢用简单的、手工编写的结构，加上许多连接行为，让权值学习来完成所有工作        
这就是机器学习版本关于先天和后天的争论，结构学习＋权值学习，交替进行＝>终极算法？    
...
## 贝叶斯学派     
- 贝叶斯定理    
当收到新论据时，它用来改变你对某个假设的信任度，如果论据和假设一致，假设成立的概率上升，反之则下降    
拉普拉斯将之变成定理，并以贝叶斯的名字命名    
先验概率－》反映的是对于将要发生的事情，你优先相信的东西    
后验概率－》是在看到一些证据后得出的    

- 贝叶斯学派认为，概率并非频率，而是一种主观程度上的信任，因此，用概率做什么由你决定，而贝叶斯推理让你做的事是：通过数据来修正之前相信的东西    

- 简化模型    
简化的假设是，在给定原因的情况下，所有结果都相互独立。——朴素贝叶斯分类器    
例如：假设你感冒了，发烧并不会改变你也咳嗽的可能性，即P（发烧、咳嗽｜感冒）＝P（发烧｜感冒）＊P（咳嗽｜感冒））    

- 马尔可夫链    
以单词识别为例，每个字母出现的概率由在它之前、与它紧接的字母来决定    
...
- “空间是所有事情没有都发生在你身上的原因，空间结构是条件独立性的一个实例”。    
- “所有生命都与其他生命互相关联”——生物学家巴里康芒纳    

- 朴素贝叶斯法、马尔可夫链、HMM都是贝叶斯网络的特殊例子    
朴素贝叶斯－原因给定时，所有结果都独立    
马尔可夫链－考虑到现在、未来，有条件的独立于过去    
HMM－假设每个观察值只依赖于对应的状态    

- 对于贝叶斯学派来说，没有所谓的真理，你有一个优先于假设的分布，在见到数据后，它变成了后验分布    

- 贝叶斯学派的观点    
选择什么表示方法由你决定，但得利用贝叶斯定理来掌握它。可以将先验分布置于任意级别的假设中（规则组、神经网络、程序），然后，在给定数据的条件下，利用假设的可能性来对其尽心更新。    

- 马尔可夫网络    
是一组特征和对应的权值，特点和权值共同定义概率分布    

- 符号学派、联结学派、进化学派、贝叶斯学派，都学习研究中的现象的显式模型，无论它是一组规则、一个多层感知器、一个基因计划、还是一个贝叶斯网络，当没有足够数据做这件事时，就会被难住。    

- 类比学派，可以从甚至小到一个例子的数据中学习，因为他们绝不会形成一种模式。    
